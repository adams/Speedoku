...
| 2026-06-17 | **`Tab`/`Shift+Tab` repurposed:** they now cycle the active *number* to the next/previous non-completed digit in the bottom selector; the arrows own all board-cell navigation. Supersedes the 2026-06-16 "Tab walks empty cells" decision. New `cycleNumber` intent; the old `"empty"` traversal is removed. See `docs/2026-06-17-tab-select-and-pill-contrast-design.md`. |
...
- **The number pad is NEVER blank — this is an invariant, not a nicety.** Every puzzle opens with a digit already selected: the **lowest non-solved digit**, aimed at its first legal cell. This holds on the run's first (tutorial) puzzle **and on every puzzle you advance into** — entering a fresh puzzle must never drop you into a no-selection state. Selection only ever moves *forward* (place → next valid cell → next incomplete digit); it must never fall back to "nothing selected" while the puzzle is unsolved. Rationale: number-first input means the digit *is* the cursor — a blank selector means a dead board (clicks no-op, no Focus highlighting), which reads as broken. Enforced by `lowestSelection()` used in `initRun` **and** `advance` (`lib/run/reduce.ts`); regression-guarded by `tests/run/reduce.test.ts` ("opens with the lowest non-solved digit…" / "a fresh puzzle starts with the lowest non-solved digit…"). **Do not reintroduce `activeDigit: null` on puzzle entry.**
- **Keyboard navigation is directional and reversible (v1 parity).** `1`–`9` select a digit; `Enter` places at the cursor. The **arrows move only between the active digit's valid cells**, *directionally*: `←`/`→` step the previous/next valid cell in **reading order** (row-major), `↑`/`↓` step the previous/next valid cell **column-wise** (column-major). Both wrap. **`Tab`/`Shift+Tab` cycle the active *number* to the next/previous non-completed digit in the bottom selector** (updated 2026-06-17 — they previously walked empty cells; the arrows now own all board navigation). The defining feel is that every keystroke lands on a usable target and you can always move the direction you're looking — and back. Arrows are `skipToNextCell {traversal:"valid", axis, dir}`; the number cycle is `cycleNumber {dir}` — both in `lib/run/reduce.ts`, regression-guarded in `tests/run/reduce.test.ts` (directional/reverse arrow tests + the `cycleNumber` block). **Do not collapse the four arrows to one forward-only action, don't drop the reverse (`←`/`↑`/`Shift+Tab`) paths, and don't revert `Tab` to empty-cell traversal** — see `docs/2026-06-17-tab-select-and-pill-contrast-design.md`.
- **Focus Mode is high-contrast: grey the invalid, brighten the valid.** With a digit selected, every cell the digit *can't* go to is dimmed; the legal target cells (and existing copies of that digit, and the cursor) stay full-strength so the placeable set pops at a glance. A digit is always selected during play (see the never-blank invariant), so Focus Mode is always on. Implemented as the `dimmed` branch in `components/board/Cell.tsx`.
...
- `RunState`: illegal-move rejected (no-op), legal placement accepted, unsolvable→death, puzzle-complete→advance, auto-advance, tutorial→play transitions, **pre-selected entry** (run start *and* every advance open with the lowest non-solved digit + its first legal cell — selector never blank), **directional cursor traversal** (`←`/`→` row-major, `↑`/`↓` column-major over valid cells; `Tab`/`Shift+Tab` forward/back over empties; all wrap).
