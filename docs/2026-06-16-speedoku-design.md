...
- **Keyboard navigation is directional and reversible (v1 parity).** `1`–`9` select a digit; `Enter` places at the cursor. The **arrows move only between the active digit's valid cells**, *directionally*: `←`/`→` step the previous/next valid cell in **reading order** (row-major), `↑`/`↓` step the previous/next valid cell **column-wise** (column-major). Both wrap. `Tab`/`Shift+Tab` walk the next/**previous** *available* (empty) cell — any empty cell, not just valid-for-digit. The defining feel is that every keystroke lands on a usable cell and you can always move the direction you're looking — and back. Modeled as `skipToNextCell {traversal, axis, dir}` in `lib/run/reduce.ts`; regression-guarded by the directional/reverse traversal tests in `tests/run/reduce.test.ts`. **Do not collapse the four arrows to one forward-only action, and don't drop the reverse (`←`/`↑`/`Shift+Tab`) paths** — that was the v1-regression we fixed.
