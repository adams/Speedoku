...
- `RunState`: illegal-move rejected (no-op), legal placement accepted, unsolvable→death, puzzle-complete→advance, auto-advance, tutorial→play transitions, **pre-selected entry** (run start *and* every advance open with the lowest non-solved digit + its first legal cell — selector never blank).
